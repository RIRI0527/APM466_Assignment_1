...
# Compute spot curve for each date
spot_results <- df_bonds %>%
  ...
  unnest(cols = c(SpotRates))

# Ensure Years to Maturity is included
spot_results <- spot_results %>% mutate(`Years to Maturity` = rep(df_bonds$`Years to Maturity`, length(unique(spot_results$Date))))

# Plot Spot Curve
ggplot(spot_results, aes(x = `Years to Maturity`, y = SpotRates, color = as.factor(Date), group = Date)) +
  geom_line(linewidth = 0.5) +
  geom_point(size = 2) +
  scale_color_manual(values = rainbow(length(unique(spot_results$Date)))) +
  labs(title = "Spot Curves from Jan 6 to Jan 17", x = "Time to Maturity", y = "Spot Rate (%)", color = "Date") +
  theme_minimal() +
